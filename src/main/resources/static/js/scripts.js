...
(function($) {
	...
	$('#inputEstado').change(function() {
		...
		$.get("findMunicipios/" + $(this).val(), function(municipio) {
			var $dropdown = $("#inputMunicipio");
			$dropdown.empty();
			$.each(municipio, function() {
				$dropdown.append($("<option/>").val(this.idMunicipio).text(this.nombre));
			});
		});
	})
